  ...
  async function handleSubmit() {
    ...
    try {
      await scheduler.assign({
        guru_id: Number(selectedTeacher.guruId),
        id: Number(selectedSchedule.value),
      });
      alert.success("Assign schedule berhasil");
      queryClient.invalidateQueries({ queryKey: ["biodata-guru"] });
    } catch (error: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={(val) => { if (!val) handleClose(); }}>
      <DialogContent className="w-full max-w-md">
        <DialogHeader>
          <DialogTitle style={{ marginTop: "18px" }}>{lang.text("assignSchedule")}</DialogTitle>
        </DialogHeader>
