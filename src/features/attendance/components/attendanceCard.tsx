  ...
  return (
    ...
    >
      <Card
        ...
        aria-label={`Attendance card for ${data.user.name ?? "Unknown"}`}
      >
        {/* 🔹 Profile Image */}
        <div className="relative w-full h-48 rounded-lg overflow-hidden shadow-sm">
          <img
            src={data.user.image || "/defaultProfile.png"}
            alt={`${data.user.name ?? "Unknown"}'s profile`}
        ...
        <CardContent className="flex flex-col p-0 pt-4">
          <h3 className="text-lg font-semibold tracking-tight border-b border-white/10 pb-3 mb-3">
            {data.user.name ?? "Tidak Tersedia"}
          </h3>
          <p className="text-sm text-gray-600 dark:text-gray-400 font-medium mb-2">
            NISN: {data.user.nisn ?? "Tidak Tersedia"}
          </p>
          ...
          >
            ...
            <Badge className="py-1 px-3 text-sm font-semibold">
              {formatJamMasuk(data.absensis?.[0]?.jamMasuk)}
            </Badge>
          </motion.div>
        </CardContent>
      </Card>
