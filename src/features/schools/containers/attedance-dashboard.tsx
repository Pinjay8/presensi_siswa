...
export const AttedancesReport = ({ selectedSchool }: AttedancesReportProps) => {
  ...
  const filteredBySchool = useMemo(() => {
    ...
    if (typeof biodata.data === "string") {
      ...
      parsedData = Array.isArray(biodata.data)
        ? biodata.data
        : [];
    }
      ...
      <div className="mt-4 mb-12">
        {
          profile?.user?.role === 'admin' && (
            <div className='absolute top-3 right-[10%] w-full flex justify-between items-center'>
              <Button variant='outline' onClick={() => handleRefecth()} className='ml-auto px-3'>
                <RefreshCcw className={turn ? 'transform rotate-[120deg] duration-200' : 'transform rotate-[-120deg] duration-200 ease-in'} />
