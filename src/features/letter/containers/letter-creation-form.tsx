...
export const LetterCreationForm = () => {
  ...
  const [signatureDataUrl, setSignatureDataUrl] = useState<string | undefined>(school.data?.ttdKepalaSekolah);
    ...
    if (school.data) {
      ...
      setSignatureDataUrl(
        school.data?.ttdKepalaSekolah
          ? school.data?.ttdKepalaSekolah.startsWith('data:image')
            ? school.data?.ttdKepalaSekolah
            : `data:image/png;base64,${school.data?.ttdKepalaSekolah}`
          : undefined
      );
    }
  }, [school.data, form]);
  ...
  async function onSubmit(data: z.infer<typeof letterUpdateFormSchema>) {
    ...
    try {
      ...
      if (data.namaKepalaSekolah && data.namaKepalaSekolah !== school.data?.namaKepalaSekolah) {
