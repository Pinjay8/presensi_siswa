...
const NavItem = React.memo(({ isCollapsed, ...props }: NavItemPropsExtended) => {
  ...
  return (
    <li className="relative">
      ...
      >
        <div className={cn("flex flex-row items-center gap-2", isCollapsed && "justify-center")}>
          {props.icon && (
            <Icon
              iconName={props.icon}
              className="sidebar-nav-item-icon h-4 w-4"
      ...
      {hasChild && visibleChild && !isCollapsed && (
        <Nav isChild items={props.items || []} mobile={props.mobile} isCollapsed={isCollapsed} />
      )}
    </li>
  );
});
...
export const Nav = React.memo(
  ({ items = [], mobile = false, isChild, isCollapsed }: NavProps & { isCollapsed?: boolean }) => {
    return (
      <ul className={cn(isChild && 'pl-4')}>
        {items?.map((item, index) => (
