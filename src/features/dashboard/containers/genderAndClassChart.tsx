...
export const StudentDemographicsCharts = ({ selectedSchool }: { selectedSchool?: string }) => {
  ...
  const { data, refetch } = useStudentPagination(studentParams); // Pastikan refetch diambil dari hook
  ...
  // Process gender data
  const genderData = students.data.reduce(
    (acc, student) => {
  ...
  const classData = classRooms.data.reduce((acc, classroom) => {
    const studentCount = classroom.biodataSiswa?.length || 0;
    const className = classroom.namaKelas?.trim() || 'Unknown';
    if (className && studentCount > 0) { // Only include valid class names with students
